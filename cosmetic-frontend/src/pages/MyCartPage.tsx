...
export default function MyCartPage() {
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      const cartItem: CartItem[] = JSON.parse(savedCart);
      setCart(cartItem);

      const fetchCosmeticDetails = async () => {
        const validCosmetic: Cosmetic[] = [];
        ...
        for (const item of cartItem) {
          try {
            ...
            if (error instanceof Error) {
              ...
              const updatedCartAfterError = cartItem.filter(
        ...
        setCosmeticDetails(validCosmetic);
        setLoading(false);
      };
